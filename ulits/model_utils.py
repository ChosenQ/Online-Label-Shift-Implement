...
class UnbiasedOnlineGradientDescentClassifier:
    ...
    def weighted_cross_entropy(logits, labels, weights):
        ...
        return -jnp.sum(weights * one_hot_labels * jax.nn.log_softmax(logits))


    def update_weight_optax(self,params, ref_features, ref_y, q_hat, source_train_marginals, optimizer, opt_state):
        ...
        lr = self.lr
        optimizer = optax.sgd(lr, momentum=0.0)  
        opt_state = optimizer.init(params)       
        loss_weights = jax.device_put(jnp.array(q_hat / source_train_marginals, dtype=jnp.float32))

        def loss_fn(params, ref_features, ref_y, weights):
            logits = model.apply(params, ref_features)['logits']  
            return weighted_cross_entropy(logits, ref_y, weights)

        # Calculate gradients
        grads, loss = jax.value_and_grad(loss_fn, argnums=0)(params, ref_features, ref_y, loss_weights)
        ...
        return new_params, opt_state, loss.item()
...
class LinearLayerRetrainedClassifier:
    ...
    def update_weight_optax(self,params, opt_state, ref_features, ref_y, weights):
        self.model.feature_extractor.eval()
        self.model.linear = nn.Linear(
                self.model.linear.in_features,
                self.model.linear.out_features,
        ...
        self.model.linear.train()
        params = self.model.linear.parameters()
        optimizer = optax.sgd(lr=0.01, momentum=0.5)  # Define learning rate and momentum
        opt_state = optimizer.init(params)
        def loss_fn(params, ref_features, ref_y, weights):
            logits = model.apply(params, ref_features)
            return weighted_cross_entropy(logits, ref_y, weights)
